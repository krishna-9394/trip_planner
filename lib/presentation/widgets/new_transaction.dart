...
class NewTransaction extends StatefulWidget {
  ...
  const NewTransaction(
      {super.key, required this.trip, required this.tripIndex});
...
class _NewTransactionState extends State<NewTransaction> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(child: BlocBuilder<TripBloc, TripState>(
      builder: (context, state) {
        if (state is LoadingUsersState) {
          ...
          return Center(
            child:
                Text("failed to the screen due to ${state.error.toString()}"),
          );
        } else {
          return const Center(
